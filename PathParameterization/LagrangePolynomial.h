...
namespace CorrGen{
    ...
    class LagrangePolynomial{
        ...
        Eigen::VectorXd d_basis(double eps) const {
            // derivative of the Lagrange interpolating polynomial basis
            Eigen::VectorXd L = basis(eps);
            Eigen::VectorXd dL(N_breaks);
            for(int i = 0; i < N_breaks; i++){
                dL[i] = 0;
                for(int j = 0; j < N_breaks; j++){
                    if(i != j){
                        dL[i] += L[i]/(break_times_[i] - break_times_[j]);
                    }
                }
            }
            return dL; 
        ...
        Eigen::VectorXd dd_basis(double eps) const {
            // second derivative of the Lagrange interpolating polynomial basis
            Eigen::VectorXd dL = d_basis(eps);
            Eigen::VectorXd ddL(N_breaks);
            for(int i = 0; i < N_breaks; i++){
                ddL[i] = 0;
                for(int j = 0; j < N_breaks; j++){
                    if(i != j){
                        ddL[i] += dL[i]/(break_times_[i] - break_times_[j]);
                    }
                }
            }
            return ddL; 
        ...
        Eigen::MatrixX<T> d_value(double eps) const {
            Eigen::VectorXd dL = d_basis(eps);
            Eigen::MatrixX<T> result(out_rows, out_cols);

            for(int i = 0; i < out_rows; i++){
                for(int j = 0; j < out_cols; j++){
                    result(i,j) = dL.transpose() * ctrl_points_(i,j);
                }
            }
        ...
        Eigen::MatrixX<T> dd_value(double eps) const {
            Eigen::VectorXd ddL = dd_basis(eps);
            Eigen::MatrixX<T> result(out_rows, out_cols);

            for(int i = 0; i < out_rows; i++){
                for(int j = 0; j < out_cols; j++){
                    result(i,j) = ddL.transpose() * ctrl_points_(i,j);
                }
            }
        ...
        std::vector<double> break_times() const {
            return break_times_;
        }
